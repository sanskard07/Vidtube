...
const registerUser = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Rollback: delete files from Cloudinary if user creation fails
    if (avatar) {
    ...
    throw new ApiError(
      500,
      "Something went wrong while registering the user. Uploaded files were deleted."
    );
  }
});
...
const logoutUser = asyncHandler(async (req, res) => {
  await User.findByIdAndUpdate(
    req.user._id,
    {
      $set: { refreshToken: undefined }, // Corrected syntax
    },
    { new: true }
  );
  const options = {
